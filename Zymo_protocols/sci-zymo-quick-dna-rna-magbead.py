def get_values(*names):
    ...
    _all_values = json.loads(
        """{"num_samples":8,"include_dnase":false,"deepwell_type":"nest_96_wellplate_2ml_deep","res_type":"nest_12_reservoir_15ml","starting_vol":400,"binding_buffer_vol":430,"wash1_vol":500,"wash2_vol":500,"wash3_vol":500,"elution_vol":50,"mix_reps":15,"settling_time":7,"park_tips":false,"tip_track":false,"flash":false,"include_bind":true,"include_wash1":true,"include_wash2":true,"include_wash3":true,"include_wash4":true,"include_dry":true,"include_elute":true}""")
...
def run(ctx):
    ...
    if park_tips:
        parkingrack = ctx.load_labware(
            'opentrons_96_filtertiprack_200ul', '1', 'tiprack for parking')
...
' + str(settling_time) + ' minutes.')
    ...
    if include_bind:
        bind(binding_buffer_vol, park=park_tips) # volume and boolean
    if include_wash1:
        wash(wash1_vol, wash1, park=park_tips)  # The variables are, volume, Boolean, boolean
    if include_wash2:
